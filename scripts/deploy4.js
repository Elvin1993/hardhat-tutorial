...
async function verify(contractAddress, args) {
  ...
  try {
    await run("verify:verify", {
      ...
      contract: "contracts/TinTinTestToken.sol:TinTinTestToken"
    })
  } catch (e) {
...
async function main() {
  ...
  const Token =await ethers.getContractFactory("TinTinTestToken");
  const MyToken = await Token.deploy();

  console.log('TinTinTestToken 部署后的合约地址是:')
